...
function dupCheckId( $obj ){
    ...
    $.ajax({
        ...
        success: function(response)
        {
            ...
            {
                frm.find('name=i_user_id]').addClass('error');
            ...
            frm.find('[name=i_user_id]').removeClass('error');
            idChk = true;
...
function deleteMember(){
    ...
    box_confirm( '정말 삭제하시겠습니까?', 'q', '', deleteMemberProc);
}
...
function deleteMemberProc(){
    ...
    $.ajax({
        url: "/apis/membership/memberDelete",
        method: "POST",
        data: frm.serialize(),
        dataType: "json",
        cache: false,
...
function modfiyConfirm(){
    box_confirm('회원정보를 수정하시겠습니까?', 'q', '', function(){
        modfiyMember(event); // event 객체 전달
    });
}

function modfiyMember(event){
    ...
    $.ajax({
        url: "/apis/membership/memberModify",
        method: "POST",
        data: frm.serialize(),
        dataType: "json",
        cache: false,
