...
function orderStatusChangeListsConfirm( status ){
    ...
    if(s_ordStatus.indexOf('PayComplete') !== -1) {
        if( $("#completeOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').length === 0 ){
            box_alert( '상태변경할 데이터를 선택해주세요.', 'e' );
            return false;
        }
        $("#completeOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').each(function(){
            idxs.push( $(this).val() );
        });
    }else if(s_ordStatus.indexOf('ProductWaiting') !== -1) {
        if( $("#prdReadyOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').length === 0 ){
            box_alert( '상태변경할 데이터를 선택해주세요.', 'e' );
            return false;
        }
        $("#prdReadyOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').each(function(){
            idxs.push( $(this).val() );
        });
    }else if(s_ordStatus.indexOf('ShippingWaiting') !== -1) {
        if( $("#shipReadyOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').length === 0 ){
            box_alert( '상태변경할 데이터를 선택해주세요.', 'e' );
            return false;
        }
        $("#shipReadyOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').each(function(){
            idxs.push( $(this).val() );
        });
    }else if(s_ordStatus.indexOf('ShippingProgress') !== -1) {
        if( $("#shippingOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').length === 0 ){
            box_alert( '상태변경할 데이터를 선택해주세요.', 'e' );
            return false;
        }
        $("#shippingOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').each(function(){
            idxs.push( $(this).val() );
        });
    }else if(s_ordStatus.indexOf('ShippingComplete') !== -1) {
        if( $("#shipCompOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').length === 0 ){
            box_alert( '상태변경할 데이터를 선택해주세요.', 'e' );
            return false;
        }
        $("#shipCompOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').each(function(){
            idxs.push( $(this).val() );
        });
    }else if(s_ordStatus.indexOf('ShippingCancelRequest') !== -1) {
        if( $("#shipCancelOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').length === 0 ){
            box_alert( '상태변경할 데이터를 선택해주세요.', 'e' );
            return false;
        }
        $("#shipCancelOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').each(function(){
            idxs.push( $(this).val() );
        });
    }else if(s_ordStatus.indexOf('ShippingCancelRequestBuyer') !== -1) {
        if( $("#otherCancelOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').length === 0 ){
            box_alert( '상태변경할 데이터를 선택해주세요.', 'e' );
            return false;
        }
        $("#otherCancelOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').each(function(){
            idxs.push( $(this).val() );
        });
    }else if(s_ordStatus.indexOf('BuyDecision') !== -1) {
        if( $("#buyDecsOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').length === 0 ){
            box_alert( '상태변경할 데이터를 선택해주세요.', 'e' );
            return false;
        }
        $("#buyDecsOrdersTable tbody").find('input[name="o_i_idx[]"]:checked').each(function(){
            idxs.push( $(this).val() );
        });
    }
