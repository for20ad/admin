...
class GoodsApi extends ApiController
{
    ...
    public function changeGoodsRealColor()
    {
        ...
        $modelParam['G_REAL_COLOR']                 = trim( _elm( $requests, 'i_colorText' ) );

        $aStatus                                    = $goodsModel->updateGoodsRealColor( $modelParam );
        $response['status']                         = 200;
    ...
    public function goodsDetailProc()
    {
        ...
        if( empty( _elm( $requests, 'i_goods_group_idxs' ) ) === false ){
            ...
            $groupDatas                             = json_encode( $originalGroupInfo, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE );
        ...
        if( _elm( $requests, 'i_sell_point_flag' ) == 'Y' ){
            ...
            foreach( _elm( $requests, 'i_discount_mb_group' ) as $g_key => $d_mb_group_idx  ){
                ...
                $groupParam['D_MB_GROUP_SV_AMT']    = _elm( _elm( $requests, 'i_discount_mb_group_amt' ), $g_key );
                $groupParam['D_MB_GOURP_SV_UNIT']   = _elm( _elm( $requests, 'i_discount_mb_group_amt_unit' ), $g_key );
        ...
        if( _elm( $requests, 'i_option_use_flag' ) == 'Y' ){
            ...
            foreach( _elm( $requests, 'i_option_keys' ) as $o_key => $option_key ){
                ...
                $optionParam['O_DAH_SPEC']          = _elm( _elm( $requests, 'i_option_spec' ), $o_key );
                $optionParam['O_PRD_BARCODE']       = _elm( _elm( $requests, 'i_option_barcode' ), $o_key );
    ...
    public function goodsDetailProc2()
    {
        $response                                   = $this->_initResponse();
        $requests                                   = $this->request->getPost();
        $files                                      = $this->request->getFiles();
        ...
        $modelParam['G_TAX_TYPE']                   = _elm( $requests, 'i_tax_type' );
        $modelParam['G_DISCOUNT_CD']                = empty( _elm( $requests, 'i_discount_cd' ) ) === false ? join( ',' , _elm( $requests, 'i_discount_cd' ) ) : '';
        $modelParam['G_GRADE_DISCOUNT_FLAG']        = empty( _elm( $requests, 'i_grade_discount_flag' ) ) === true ? 'N': 'Y';
        #------------------------------------------------------------------
        # TODO: 개별 포인트설정일 경우 등급데이터 세팅
        #------------------------------------------------------------------
        $modelParam['G_SELL_POINT_FLAG']            = _elm( $requests, 'i_sell_point_flag' );
        ...
        if( _elm( $requests, 'i_option_use_flag' ) == 'Y' ){
            ...
            $modelParam['G_OPTION_COMBINATION_FLAG']= _elm( $requests, 'i_option_combination_flag' );
            $modelParam['G_OPTION_NAMES']           = implode( '|', _elm( $requests, 'i_option_group_keys' )?? [] );

            if( empty( _elm( $requests, 'i_tag_data' ) ) === false ){
                $modelParam['G_OPTION_INFO']        = json_encode( json_decode( _elm( $requests, 'i_tag_data' ), true), JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE);
            }else{
                $modelParam['G_OPTION_INFO']        = NULL;
            }

            #------------------------------------------------------------------
            # TODO: 조합형 여부에 따른 분기처리
            #------------------------------------------------------------------
            if( _elm( $requests, 'i_option_combination_flag' ) == 'Y' ){

                if( empty( _elm( $requests, 'i_option_idx' ) ) === true ){
                    #------------------------------------------------------------------
                    # TODO: 기존 데이터 삭제
                    #------------------------------------------------------------------
                    $oDelStatus                     = $goodsModel->deleteData( 'GOODS_OPTIONS', ['field'=>'O_GOODS_IDX', 'idx'=>_elm( $requests, 'i_goods_idx' ) ] );
                ...
                // i_option_values 관련 모든 배열을 수집
                $option_keys = array_filter(array_keys($requests), function ($key) {
                    return preg_match('/^i_option_value\d*$/', $key);  // i_option_values, i_option_values2, i_option_values3...
                });

                // 배열 정렬 (i_option_value, i_option_value2 순서 보장)
                natsort($option_keys);
                $option_keys                        = array_values($option_keys); // ✅ 키를 0부터 순차적으로 재정렬

                // 각 옵션 값 배열을 순회하면서 조합 생성
                $num_rows                           = count($requests['i_option_value']); // i_option_values의 개수로 루프 횟수 결정

                for ($i = 0; $i < $num_rows; $i++) {
                    $optionParam                    = [];
                    $counter                        = 0; // ✅ 별도의 카운터 사용

                    foreach ($option_keys as $key) {
                        $value                      = isset($requests[$key][$i]) ? $requests[$key][$i] : '';

                        // O_VALUES, O_VALUES2 등으로 변수명 설정
                        $var_name                   = 'O_VALUES' . ($counter > 0 ? $counter + 1 : '');

                        // 배열에 추가
                        $optionParam[$var_name]     = $value;
                        $counter++; // 카운터 증가
                    }
                    $optionIdx = isset($requests['i_option_idx'][$i]) ? $requests['i_option_idx'][$i] : null;

                    // 추가적인 정보 (예: 스펙, 바코드 등)
                    $optionParam['O_DAH_SPEC']      = isset($requests['i_option_spec'][$i]) ? $requests['i_option_spec'][$i] : '';
                    $optionParam['O_PRD_BARCODE']   = isset($requests['i_option_barcode'][$i]) ? $requests['i_option_barcode'][$i] : '';
                    $optionParam['O_STOCK']         = isset($requests['i_option_stock'][$i]) ? $requests['i_option_stock'][$i] : '';
                    $optionParam['O_ADD_PRICE']     = isset($requests['i_option_add_price'][$i]) ? preg_replace('/,/','', $requests['i_option_add_price'][$i] ) : '';
                    $optionParam['O_VIEW_STATUS']   = isset($requests['i_option_status'][$i]) ? $requests['i_option_status'][$i] : '';
        ...
        $modelParam['G_STOCK_CNT']                  = _elm( $requests, 'i_goods_stock_flag' ) == 'Y' ? _elm( $requests, 'i_goods_stock' ) : '999999';
        $modelParam['G_SAFETY_STOCK']               = _elm( $requests, 'i_goods_safe_stock' );
        $modelParam['G_IS_RESTOCK_ALIM_FLAG']       = _elm( $requests, 'i_is_restock_alim_flag', 'N', true );
        #------------------------------------------------------------------
        # TODO: 텍스트 옵션 세팅
        #------------------------------------------------------------------
        if( _elm( $requests, 'i_text_option_use_flag' ) == 'Y' ){
    ...
    public function copyGoods()
    {
        ...
        $modelParam['G_DAHAE_P_CODE']               = _elm( $goodsInfo, 'G_DAHAE_P_CODE' );
        ...
        $modelParam['G_REAL_COLOR']                 = _elm( $goodsInfo, 'G_REAL_COLOR' );
        ...
        $modelParam['G_GRADE_DISCOUNT_FLAG']        = _elm( $goodsInfo, 'G_GRADE_DISCOUNT_FLAG' );
        ...
        $modelParam['G_OPTION_NAMES']               = _elm( $goodsInfo, 'G_OPTION_NAMES' );
        $modelParam['G_OPTION_INFO']                = _elm( $goodsInfo, 'G_OPTION_INFO' );
        $modelParam['G_OPTION_COMBINATION_FLAG']    = _elm( $goodsInfo, 'G_OPTION_COMBINATION_FLAG' );
        $modelParam['G_SAFETY_STOCK']               = _elm( $goodsInfo, 'G_SAFETY_STOCK' );
        $modelParam['G_IS_RESTOCK_ALIM_FLAG']       = _elm( $goodsInfo, 'G_IS_RESTOCK_ALIM_FLAG' );
        ...
        if( empty( $dcGroupData ) === false ){
            foreach( $dcGroupData as $gKey => $group ){
                ...
                $groupParam['D_MB_GROUP_SV_AMT']    = _elm( $group, 'D_MB_GROUP_SV_AMT' );
                $groupParam['D_MB_GOURP_SV_UNIT']   = _elm( $group, 'D_MB_GOURP_SV_UNIT' );
        ...
        if( empty( $optionData ) === false ){
            foreach( $optionData as $oKey => $option  ){
                ...
                $optionParam['O_DAH_SPEC']          = _elm( $option, 'O_DAH_SPEC' );
    ...
    public function goodsRegisterProc()
    {
        ...
        if( _elm( $requests, 'i_sell_point_flag' ) == 'Y' ){
            ...
            foreach( _elm( $requests, 'i_discount_mb_group' ) as $g_key => $d_mb_group_idx  ){
                ...
                $groupParam[$g_key]['D_MB_GROUP_SV_AMT']    = _elm( _elm( $requests, 'i_discount_mb_group_amt' ), $g_key );
                $groupParam[$g_key]['D_MB_GOURP_SV_UNIT']   = _elm( _elm( $requests, 'i_discount_mb_group_amt_unit' ), $g_key );
            ...
            if( _elm( $requests, 'i_option_combination_flag' ) == 'Y' ){
                ...
                for ($i = 0; $i < $num_rows; $i++) {
                    ...
                    foreach ($option_keys as $key) {
                        ...
                        // 배열에 추가
                        $optionParam[$i][$var_name] = $value;
                        $counter++; // 카운터 증가
                    }
                    $optionIdx = isset($requests['i_option_idx'][$i]) ? $requests['i_option_idx'][$i] : null;

                    // 추가적인 정보 (예: 스펙, 바코드 등)
                    $optionParam[$i]['O_DAH_SPEC']  = isset($requests['i_option_spec'][$i]) ? $requests['i_option_spec'][$i] : '';
                    $optionParam[$i]['O_PRD_BARCODE']= isset($requests['i_option_barcode'][$i]) ? $requests['i_option_barcode'][$i] : '';
                    $optionParam[$i]['O_STOCK']     = isset($requests['i_option_stock'][$i]) ? $requests['i_option_stock'][$i] : '';
                    $optionParam[$i]['O_ADD_PRICE'] = isset($requests['i_option_add_price'][$i]) ? preg_replace('/,/','', $requests['i_option_add_price'][$i] ) : '';
                    $optionParam[$i]['O_VIEW_STATUS']= isset($requests['i_option_status'][$i]) ? $requests['i_option_status'][$i] : '';

                    $optionParam[$i]['O_CREATE_AT'] = date( 'Y-m-d H:i:s' );
                    $optionParam[$i]['O_CREATE_IP'] = $this->request->getIPAddress();
                    $optionParam[$i]['O_CREATE_MB_IDX'] = _elm( $this->session->get('_memberInfo') , 'member_idx' );
                    $optionParam[$i]['O_GOODS_IDX'] = _elm( $requests, 'i_goods_idx' );
                }
            }else{
                foreach( _elm( $requests, 'i_option_keys' ) as $o_key => $option_key ){
                    $optionParam                         = [];
                    ...
                    $optionParam[$o_key]['O_DAH_SPEC']   = _elm( _elm( $requests, 'i_option_spec' ), $o_key );
                    $optionParam[$o_key]['O_PRD_BARCODE']= _elm( _elm( $requests, 'i_option_barcode' ), $o_key );
                    ...
                    $optionParam[$o_key]['O_GOODS_IDX']     = _elm( $requests, 'i_goods_idx' );
    ...
    public function getGoodsRestockAlimLists( $param = [] ){
        $response                                   = $this->_initResponse();
        $owensView                                  = new OwensView();
        $goodsModel                                 = new GoodsModel();
        $requests                                   = _trim($this->request->getPost());
        ...
        if( empty( _elm( $requests , 's_keyword' ) ) === false )
        {
            switch( _elm( $requests , 's_condition' ) )
            {
                ...
                case 'member_id' :
                    ...
                    break;
                case 'member_name' :
                    $modelParam['MB_NAME']          = _elm( $requests , 's_keyword' );
                    break;
            }
        }
        ...
        ###########################################################
        $aLISTS_RESULT                              = $goodsModel->getGoodsRestockLists( $modelParam );

        $lists                                      = _elm( $aLISTS_RESULT, 'lists' );

        if( empty( $lists ) === false ){
            foreach( $lists as $lKey => &$aList ){
                $_options                           = explode( ',', _elm( $aList, 'A_OPTIONS_IDX' ) );
                $optionInfos                        = $goodsModel->getGoodsOptionsByIdxs( $_options );
                $_optionText                        = explode( ',', _elm( $aList, 'A_OPTIONS_TEXT' ) );

                if( empty( $optionInfos ) === false  ){
                    foreach( $optionInfos as $oKey => $option ){
                        $_optionText[$oKey]         = $_optionText[$oKey].' : '._elm( $option, 'O_STOCK' ).'EA';
                    }

                    $aList['A_OPTIONS_TEXT']        = implode( ',', $_optionText );
                }
            }
            unset( $aList );
        }
        ...
        if (_elm($requests, 'page_return') === true || $this->request->isAjax() === true)
        {
            ...
            $owensView->setViewDatas( $view_datas );
            $page_datas['lists_row']                = view( '\Module\goods\Views\goods\restock_lists_row' , ['owensView' => $owensView] );
    ...
    public function goodsRestockDetailList()
    {
        $response                                   = $this->_initResponse();
        $owensView                                  = new OwensView();
        $goodsModel                                 = new GoodsModel();
        ...
        $modelParam                                 = [];
        $modelParam['A_PRD_IDX']                    = _elm( $requests, 's_goods_idx' );
        ...
        if( empty( _elm( $requests , 's_send_gbn' ) ) === false )
        {
            if( _elm( $requests , 's_send_gbn' ) == 'Y' ){
                $modelParam['A_SEND_GBN']           = 'Y';
            }else if( _elm( $requests , 's_send_gbn' ) == 'Y' ){
                $modelParam['A_SEND_GBN']           = 'N';
            }
        }
        $modelParam['order']                        = 'A_CREATE_AT DESC';

        $aLISTS_RESULT                              = $goodsModel->getGoodsRestockAlimDetailLists( $modelParam );
        $lists                                      = _elm( $aLISTS_RESULT, 'lists' );
        $total_count                                = _elm( $aLISTS_RESULT, 'totalCount' );

        $aData                                      = $goodsModel->getGoodsRestockAlimDetailInfo( $modelParam );
        ...
        if (_elm($requests, 'page_return') === true || $this->request->isAjax() === true)
        {
            ...
            if( empty( $aData ) === false ){
                // 상품 옵션 데이터 조회
                $aGoodsOptions = $goodsModel->getGoodsOptions(_elm($aData, 'A_PRD_IDX'));

                // 옵션 텍스트 분리
                $_optionText = explode(',', _elm($aData, 'A_OPTIONS_TEXT'));
                $optionTextFinal = [];

                // 옵션 데이터를 aKey 기준으로 매핑하여 처리
                foreach ($aGoodsOptions as $aKey => $option) {
                    foreach ($_optionText as $pKey => $_text) {

                        // 기존 값이 중복되지 않도록 초기화
                        $optionTextFinal[$aKey] = "{$_text} [" . _elm($option, 'O_STOCK', 0, true) . "EA]";

                        if (_elm($aData, 'G_OPTION_COMBINATION_FLAG') == 'Y') {
                            // O_VALUES ~ O_VALUES5를 배열로 정리
                            $optionValues = implode(' ', array_filter([
                                $option['O_VALUES'],
                                $option['O_VALUES2'],
                                $option['O_VALUES3'],
                                $option['O_VALUES4'],
                            ...
                            // 옵션 텍스트를 공백 기준으로 분할하여 비교
                            $textParts = explode(' ', trim($_text));

                            // 옵션 값이 완전히 일치하면 노란색 적용
                            if ($optionValues === $textParts) {
                                $optionTextFinal[$aKey] = "<span style='>{$optionValues} [" . _elm($option, 'O_STOCK') . "EA]</span>";
                            } else {
                                $optionTextFinal[$aKey] = "<span style='background-color:yellow'>{$optionValues} [" . _elm($option, 'O_STOCK', 0, true) . "EA]</span>";
                                if( empty( _elm( $aData, 'G_IDX' ) ) === true ){

                                    $optionTextFinal[$aKey] = "<span style='background-color:#FF0000'>{$optionValues} [" . _elm($option, 'O_STOCK', 0, true) . "EA]</span>";
                                }
                            }
                            break; // 불필요한 중복 실행 방지
                        } else {
                            // 0번째 값은 O_KEYS, 1번째 값은 O_VALUES로 비교
                            $textParts = explode(' ', trim($_text) );

                            $keyMatch = !empty($textParts[0]) && $textParts[0] == $option['O_KEYS'];
                            $valueMatch = !empty($textParts[1]) && $textParts[1] == $option['O_VALUES'];

                            if ($keyMatch && $valueMatch) {
                                $optionTextFinal[$aKey] = "<span style=''>{$option['O_KEYS']} {$option['O_VALUES']} [" . _elm($option, 'O_STOCK') . "EA]</span>";
                            } else {
                                $optionTextFinal[$aKey] = "<span style='background-color:yellow'>{$option['O_KEYS']} {$option['O_VALUES']} [" . _elm($option, 'O_STOCK', 0, true) . "EA]</span>";

                                if( empty( _elm( $aData, 'G_IDX' ) ) === true ){

                                    $optionTextFinal[$aKey] = "<span style='background-color:#FF0000'>{$option['O_KEYS']} {$option['O_VALUES']} [" . _elm($option, 'O_STOCK', 0, true) . "EA]</span>";
                                }
                            }
                            break; // 불필요한 중복 실행 방지
                        }
                ...
                // 최종 결과를 문자열로 조합
                $aData['A_OPTIONS_TEXT'] = implode(',', $optionTextFinal);
            ...
            if( empty( $lists ) === false ){
                foreach( $lists as $lKey => &$aList ){
                    $aList['MB_MOBILE_NUM']         = $this->_aesDecrypt( _elm( $aList, 'MB_MOBILE_NUM' ) );
                }
                unset( $aList );
            }
            ...
            $paging_param['per_page']               = $per_page;
            $paging_param['total_rows']             = $total_count;
            $paging_param['base_url']               = rtrim( _link_url( '/goods/goodsRestockSub' ), '/');
            $paging_param['ajax']                   = true;
            $paging_param['cur_page']               = $page;
            ...
            $page_datas['detail']                   = view( '\Module\goods\Views\goods\restock_pop_lists' , ['owensView' => $owensView] );
    ...
    public function getPopGoodsRestockAlimDetailLists()
    {
        $response                                   = $this->_initResponse();
        $owensView                                  = new OwensView();
        $goodsModel                                 = new GoodsModel();
        ...
        if( empty( _elm( $requests , 's_send_gbn' ) ) === false )
        {
            if( _elm( $requests , 's_send_gbn' ) == 'Y' ){
                $modelParam['A_SEND_GBN']           = 'Y';
            }else if( _elm( $requests , 's_send_gbn' ) == 'N' ){
                $modelParam['A_SEND_GBN']           = 'N';
            }
        }
        $modelParam['order']                        = 'A_CREATE_AT DESC';
        ...
        if (_elm($requests, 'page_return') === true || $this->request->isAjax() === true)
        {
            ...
            $view_datas                             = [];
            if( empty( $aData ) === false ){
                $_options                           = explode( ',', _elm( $aData, 'A_OPTIONS_IDX' ) );
                $optionInfos                        = $goodsModel->getGoodsOptionsByIdxs( $_options );
                $_optionText                        = explode( ',', _elm( $aData, 'A_OPTIONS_TEXT' ) );
                //print_R( $aData );
                if( empty( $optionInfos ) === false  ){
                    ...
                    $aData['A_OPTIONS_TEXT']        = implode( ',', $_optionText );
            ...
            $page_datas['lists_row']                = view( '\Module\goods\Views\goods\restock_pop_lists_row' , ['owensView' => $owensView] );
    ...
    public function sendRestockAlim()
    {
        $response                                   = $this->_initResponse();
        $goodsModel                                 = new GoodsModel();
        $requests                                   = _trim($this->request->getPost());
        ...
        $kakao                                      = new KakaoLib;
        $talkParam                                  = [];
        $template_code                              = array_search( '재입고 알림', $kakaoTemplateInfo);
        ...
        foreach( _elm( $requests, 'i_idx' ) as $iKey => $a_idx ){
            $aData                                  = $goodsModel->getRestockAlimData( $a_idx );
            if( empty( $aData ) === true ){
                $response['status']                 = 400;
                $response['error']                  = 400;
                $response['messages']               = '존재하지 않는 데이터입니다.';
            ...
            if( empty( $aData ) === false ){
                $_options                           = explode( ',', _elm( $aData, 'A_OPTIONS_TEXT' ) );
                if( _elm( $aData, 'G_OPTION_COMBINATION_FLAG' ) == 'Y' ){
                    $optionInfos                    = $goodsModel->getGoodsOptionsCombinationByOptionText( _elm( $aData, 'A_PRD_IDX' ), $_options );
                }else{
                    $optionInfos                    = $goodsModel->getGoodsOptionsByOptionText( $_options );
                }
                //$optionInfos                      = $goodsModel->getGoodsOptionsByIdxs( $_options );
                ...
                if( empty( $optionInfos ) === false  ){
                    foreach( $optionInfos as $oKey => $option ){
                        if( _elm( $aData, 'G_OPTION_COMBINATION_FLAG' ) == 'Y' ){
                            ...
                            $option_name            = _elm($option, 'O_KEYS'). ' ' ._elm($option, 'O_VALUES');
                        }


                        // 옵션 이름을 공백으로 구분하여 합침

                        $_optionText[$oKey]         = $option_name.' : '._elm( $option, 'O_STOCK' ).'EA';
                    }

                    $aData['A_OPTIONS_TEXT']        = implode( PHP_EOL, $_optionText );
            ...
            $kakao_param                            = [
                ...
                'CALLPHONE'                         => $this->_aesDecrypt( _elm( $aData, 'MB_MOBILE_NUM' ) ),
                'REQTIME'                           => '00000000000000',
                'MB_IDX'                            => _elm( $aData, 'A_MB_IDX' ),
                'FCM_SEND'                          => true,
                'typeValues'                        => [
                    'mall_name'                     => '산수유람',
                    'user_name'                     => _elm( $aData, 'MB_NM' ),
                    'product_name'                  => _elm( $aData, 'A_PRD_NAME' ),
                    'option_info'                   => _elm( $aData, 'A_OPTIONS_TEXT' ),
                    'detail_url'                    => shop_url().'',
                ]
            ];

            try {
                $aStatus                            = $kakao->sendATalk($kakao_param);
            }
            //알림톡발송 임시 주석처리
            catch(\Exception $e) {
                $response['status']                 = '400';
            ...
            $aStatus                                = $goodsModel->restockAlimChangeStatus( _elm( $aData, 'A_IDX' ) );

            if ( $this->db->transStatus() === false || $aStatus === false ) {
                $this->db->transRollback();
                $response['status']                 = 400;
                $response['messages']               = '상태 업데이트 처리중 오류발생.. 다시 시도해주세요.';
                return $this->respond( $response );
            }
            #------------------------------------------------------------------
            # TODO: 관리자 로그남기기 S
            #------------------------------------------------------------------
            $logParam                               = [];
            $logParam['MB_HISTORY_CONTENT']         = '상품 재입고 일림 send sms - data:'.json_encode( $aData, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE ) ;
            $logParam['MB_IDX']                     = _elm( $this->session->get('_memberInfo') , 'member_idx' );
    ...
    public function getDahaeCate()
    {
        ...
        ###########################################################
        $aLISTS_RESULT                              = $goodsModel->getDahaeCateLists( $modelParam );
        ...
        if (!empty($lists)) {
            foreach ($lists as $lKey => $data) {
                ...
                $result[] = '<a href="javascript:$(\'#frm_search [name=cate]\').val(\'' . _elm($data, 'C_CATE_BIG') . '\');getSearchList(1)">' . _elm($data, 'C_CATE_BIG') . '</a>';
                $result[] = '<a href="javascript:$(\'#frm_search [name=cate]\').val(\'' . _elm($data, 'C_CATE_MID') . '\');getSearchList(1)">' . _elm($data, 'C_CATE_MID') . '</a>';
                $result[] = '<a href="javascript:$(\'#frm_search [name=cate]\').val(\'' . _elm($data, 'C_CATE_SMALL') . '\');getSearchList(1)">' .  _elm($data, 'C_CATE_SMALL') . '</a>';
                $lists[$lKey]['cates']         = implode(' < ',$result);
            }
        }
    ...
    public function updateDahaeCate()
    {
        $response                                   = $this->_initResponse();
        $requests                                   = _trim($this->request->getPost());
        $categoryModel                              = new CategoryModel();
        ...
        $lastCategory = []; // 마지막 카테고리 데이터를 저장할 배열
        $modelParam['C_RESULT_LOCAL_CATE_NM']       = '';
        $modelParam['C_RESULT_LOCAL_CATE_CD']       = '';
        $cateCodeArray                              = []; // 카테고리 코드 저장 배열
        if (!empty( $textValueArr )) {
            $n = 0;
            foreach ($textValueArr as $textValue) {
            ...
            $modelParam['C_RESULT_LOCAL_CATE_NM']               = _elm( $requests, 'textValue' );
            $modelParam['C_RESULT_LOCAL_CATE_CD']               = implode(',', $cateCodeArray); // 카테고리 코드를 콤마로 조인하여 저장
        }
        ...
        $aStatus                                    = $goodsModel->updateDahaeCate( $modelParam );
