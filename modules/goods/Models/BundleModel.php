...
class BundleModel extends Model
{
    ...
    public function getHotKeywordLists( $param = [] )
    {
        ...
        $builder                                    = $this->db->table( 'GOODS_G_HOTKEYWORD T' );
        $builder->select( 'T.*' );
        $builder->select( 'IFNULL(TD.CNT, 0) as GOODS_CNT' );
        $builder->join( '( SELECT COUNT( AD_IDX ) CNT, AD_P_IDX  FROM GOODS_G_HOTKEYWORD_DETAIL WHERE 1=1 GROUP BY AD_P_IDX ) TD', 'TD.AD_P_IDX = T.A_IDX', 'left' );
        if( empty( _elm( $param, 'A_TITLE' ) ) === false ){
            $builder->like( 'T.A_TITLE', _elm( $param, 'A_TITLE' ), 'both' );
        }
    ...
    public function insertHotKeyword( $param = [] )
    {
        $aReturn                                    = false;
        if( empty( $param ) === true ){
            return $aReturn;
        ...
        $builder                                    = $this->db->table( 'GOODS_G_HOTKEYWORD' );
        $builder->set( 'A_TITLE',                   _elm( $param, 'A_TITLE' ) );
        $builder->set( 'A_LIMIT',                   _elm( $param, 'A_LIMIT' ) );
        $builder->set( 'A_SORT',                    _elm( $param, 'A_SORT' ) );
        $builder->set( 'A_PERIOD_START_AT',         _elm( $param, 'A_PERIOD_START_AT' ) );
    ...
    public function getHotkeywordSortMax()
    {
        $aReturn                                    = [];
        $builder                                    = $this->db->table( 'GOODS_G_HOTKEYWORD' );
        $builder->select( 'MAX(A_SORT) max' );
    ...
    public function hotKeywordDataByIdx( $a_idx )
    {
        $aReturn                                    = [];
        if( empty( $a_idx ) === true ){
            return $aReturn;
    ...
    public function getHotKeywordDetailLists( $a_idx )
    {
        $aReturn                                    = [];
        if( empty( $a_idx ) === true ){
            return $aReturn;
        }
        $builder                                    = $this->db->table( 'GOODS_G_HOTKEYWORD_DETAIL' );
        $builder->where( 'AD_P_IDX',                $a_idx );
        $builder->orderBy( 'AD_SORT',               'ASC' );
    ...
    public function updateHotKeyword( $param = [] )
    {
        $aReturn                                    = false;
        if( empty( $param ) === true ){
            return $aReturn;
    ...
    public function deleteHotKeywordDetailGoods( $p_idx, $goods_idx )
    {
        $aReturn                                    = false;
    ...
    public function insertHotKeywordDetailGoods( $param = [] )
    {
        $aReturn                                    = false;
        if( empty( $param ) === true ){
            return $aReturn;
    ...
    public function setHotKeywordSort( $param = [] )
    {
        $aReturn                                    = false;
        if( empty( $param ) === true ){
            return $aReturn;
    ...
    public function updateHotKeywordDetailGoods( $param = [] )
    {
        $aReturn                                    = false;
        if( empty( $param ) === true ){
            return $aReturn;
    ...
    public function getHotBrandLists()
    {
        $aReturn                                    = [];

        $builder                                    = $this->db->table( 'GOODS_G_HOTBRAND' );
        $builder->orderBy( 'A_SORT',                'ASC' );
    ...
    public function deleteHotBrandGoods( $idxs )
    {
        $aReturn                                    = false;
        ...
        $builder                                    = $this->db->table( 'GOODS_G_HOTBRAND' );
        $builder->whereIn( 'A_BRAND_IDX',           $idxs );

        $aReturn                                    = $builder->delete();
        return $aReturn;
    }
    public function updateHotBrandGoods( $param = [] )
    {
        $aReturn                                    = false;
        if( empty( $param ) === true ){
            return $aReturn;
        ...
        $builder->set( 'A_UPDATE_IP',               _elm( $param, 'A_UPDATE_IP' ) );
        $builder->set( 'A_UPDATE_MB_IDX',           _elm( $param, 'A_UPDATE_MB_IDX' ) );
        $builder->where( 'A_BRAND_IDX',             _elm( $param, 'A_BRAND_IDX' ) );

        $aReturn                                    = $builder->update();
        return $aReturn;
    }

    public function insertHotBrandGoods( $param = [] )
    {
        $aReturn                                    = false;
        if( empty( $param ) === true ){
            return $aReturn;
        ...
        $builder                                    = $this->db->table( 'GOODS_G_HOTBRAND' );
        $builder->set( 'A_BRAND_IDX',               _elm( $param, 'A_BRAND_IDX' ) );
        $builder->set( 'A_SORT',                    _elm( $param, 'A_SORT' ) );
        $builder->set( 'A_LIMIT',                   _elm( $param, 'A_LIMIT' ) );
        $builder->set( 'A_CREATE_AT',               _elm( $param, 'A_CREATE_AT' ) );
        $builder->set( 'A_CREATE_IP',               _elm( $param, 'A_CREATE_IP' ) );
