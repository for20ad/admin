...
<input type="hidden" name="g_name" id="g_name" value="">
<?php echo form_close();?>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content" style="max-height:90vh;display:flex;flex-direction: column;width:80vh">
            <div class="modal-header">
                <h5 class="modal-title" id="iconsModalLabel">등록/수정</h5>
...
<!-- Modal E-->
<script>
function openLayer( idx, id ){
    let data = '';
    let url  = '/apis/goods/dahaeOptionCodeRegister';

    if( idx != '' ){

        data = 'idx='+idx;
        url  = '/apis/goods/dahaeOptionCodeDetail';
    }
    $.ajax({
        url: url,
        type: 'post',
...
$(document).on('blur', '[name=d_result]', function(){
    if( $.trim( $(this).val() ) != $.trim( $(this).data('org-value') ) ){
        updateDahaeOptionCode( $(this).closest('tr').data('idx'), $(this).val() );
    }
});
function updateDahaeOptionCode( idx, textValue ){

    var data = 'idx='+idx+'&textValue='+textValue;
    var url  = '/apis/goods/updateDahaeOptionCode';
...
function deleteDahaeOptionCodeConfirm( idx )
{
    box_confirm( '다해 옵션 매핑 코드를 삭제 하시겠습니까?', 'q', '', deleteDahaeOptionCode, idx );
}
function deleteDahaeOptionCode( idx )
{

    var data = 'idx='+idx;
    var url  = '/apis/goods/deleteDahaeOptionCode';
...
<?php
$owensView->setFooterJs('/assets/js/goods/dahae/option_lists.js');
$owensView->setFooterJs('/assets/js/goods/dahae/option_register.js');
$owensView->setFooterJs('/assets/js/goods/dahae/option_detail.js');
