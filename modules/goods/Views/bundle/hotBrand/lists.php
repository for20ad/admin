<?php
    ...
    $aBrandLists    = _elm( $pageDatas, 'brandLists', [] );
    $brandIdxs      = _elm( $pageDatas, 'brandIdxs', [] );
            ...
            <div style="border:1px solid #E6E7E9; border-top: 0px; border-radius:0 0 4px 4px">
                <div class="table-responsive">
                    <table class="table table-vcenter" id="gListsTable">
                        ...
                        <tbody id="brandSort">
                            <?php
                            if( empty( $aBrandLists ) === false ){
                                foreach( $aBrandLists as $key => $vData ){
                            ?>
                                <tr data-idx="<?php echo _elm( $vData , 'C_IDX' )?>" onmouseover="$(this).find('.group-move-icons').show()" onmouseout="$(this).find('.group-move-icons').hide()">
                                    <input type="hidden" name="i_group_brand_idxs[]" value="<?php echo _elm( $vData , 'C_IDX' )?>">
                                    <td>
                                    ...
                                    <td class="body2-c nowrap">
                                        ...
                                        <?php
                                            $setParam = [
                                                'name' => 'i_group_brand_idxs[]',
                                                'id' => 'i_group_brand_idxs'._elm( $vData, 'C_IDX'),
                                    ...
                                    <td class="body2-c nowrap"><img src="/<?php echo _elm( $vData, 'C_BRAND_PC_IMG' )?>" width="80" loading="lazy"></td>
                                    <td class="body2-c nowrap"><?php echo  _elm( $vData , 'C_BRAND_NAME' )?></td>
                                    <td class="body2-c nowrap">
...
<script>
let productPcikList = [];
let brandPickList = <?php echo json_encode( $brandIdxs ); ?>;
let addBrandPickList = [];
...
$('#brandSort').sortable({
    ...
    update: function(event, ui) {
        var idsInOrder = $('#brandSort').sortable('toArray',{ attribute : 'data-idx'});

        console.log(idsInOrder);
        $('#brandSort tr').each(function(index) {
...
function removeLine( id )
{
    $('#' + id + ' tbody input[type="checkbox"]:checked').each(function() {
        ...
        var index = brandPickList.indexOf(targetIdx); // 해당 값의 인덱스를 찾습니다.
        if (index !== -1) {
            brandPickList.splice(index, 1); // 해당 인덱스에서 1개 요소를 제거합니다.
        }
...
function openDataLayer( gbn ){
    ...
    let url = '/apis/goods/getPopBrandLists';
    let data='picLists='+brandPickList;
    $.ajax({
        url: url,
        type: 'post',
        data: data,
...
function addItem( id ){
    let brand_idxs = getCheckedItems();
    $.ajax({
        url: '/apis/goods/brandAddRows',
        type: 'post',
        data: 'brandIdxs='+brand_idxs+'&targetId='+id,
        processData: false,
        cache: false,
        beforeSend: function() {
            $('#preloader').show();
        ...
        success: function(response) {
            ...
            brand_idxs.forEach(function(p_idx) {
                brandPickList.push(p_idx);
            });

            console.log( brandPickList );
...
function frmRegister() {
    ...
    $.ajax({
        url: '/apis/goods/hotBrandRegister',
        method: 'POST',
        data: formData,
        dataType: 'json',
        processData: false,
...
<!-- Form List Modal E-->
<?php
$owensView->setFooterJs('/assets/js/goods/bundle/hotBrand/lists.js');
