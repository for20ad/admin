...
<tr data-idx="<?php echo _elm( $vData , 'A_IDX' )?>">
    ...
    <td class="body2-c nowrap">
        <a href='#' onclick="javascript:openLayer('<?php echo _elm( $vData , 'A_IDX' ) ?>', 'dataModal')">
            <?php echo date( 'Y-m-d H:i', strtotime( _elm( $vData, 'A_PERIOD_START_AT' ) ) )?> ~ <?php echo date( 'Y-m-d H:i', strtotime( _elm( $vData, 'A_PERIOD_END_AT' ) ) )?>
        </a>
    </td>
    <td class="body2-c nowrap"><a href="<?php echo _link_url( '/goods/timeSaleDetail/'._elm( $vData , 'A_IDX' ) )?>"><?php echo _elm( $vData, 'GOODS_CNT' ) ?></a></td>
