<!DOCTYPE html>
<html lang="en">

<head>
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" />
    <link rel="stylesheet" type="text/css" href="/plugins/bootstrap-5/css/bootstrap.css" />
    <link href="/dist/css/tabler.css" rel="stylesheet" />
    <link href="/dist/css/tabler-flags.css" rel="stylesheet" />
    <link href="/dist/css/tabler-payments.css" rel="stylesheet" />
    <link href="/dist/css/tabler-vendors.css" rel="stylesheet" />
    <link href="/dist/css/style.css" rel="stylesheet" />
    <link rel="shortcut icon" href="/dist/favicon.ico" type="image/x-icon">
    <link rel="icon" href="/dist/favicon.ico" type="image/x-icon">
    <link rel="stylesheet" type="text/css" href="/plugins/common/common.css" />
</head>

<body>
    <!-- 모바일 헤더 -->
    <div class="m-header">
        <!-- 메뉴 아이콘 -->
        <svg id="header-sider-bar-toggle-btn" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
            viewBox="0 0 24 24" fill="none">
            <path d="M4 6H20M4 12H20M4 18H20" stroke="black" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round" />
        </svg>
        <!-- 로고 -->
        <div>
            <img src="../../dist/img/logo_blue.png" width="110" alt="산수유람" class="navbar-brand-image">
        </div>
        <div class="d-flex gap-2">
            <!-- 타이머 -->
            <button class="btn-sm btn-gray d-flex align-items-center" style="pading: 8px">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    <path d="M5.41699 5.83331H14.5837" stroke="white" stroke-width="1.5" stroke-linecap="round"
                        stroke-linejoin="round" />
                    <path
                        d="M5 16.6667V15C5 13.6739 5.52678 12.4021 6.46447 11.4645C7.40215 10.5268 8.67392 10 10 10C11.3261 10 12.5979 10.5268 13.5355 11.4645C14.4732 12.4021 15 13.6739 15 15V16.6667C15 16.8877 14.9122 17.0996 14.7559 17.2559C14.5996 17.4122 14.3877 17.5 14.1667 17.5H5.83333C5.61232 17.5 5.40036 17.4122 5.24408 17.2559C5.0878 17.0996 5 16.8877 5 16.6667Z"
                        stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    <path
                        d="M5 3.33333V5C5 6.32608 5.52678 7.59785 6.46447 8.53553C7.40215 9.47322 8.67392 10 10 10C11.3261 10 12.5979 9.47322 13.5355 8.53553C14.4732 7.59785 15 6.32608 15 5V3.33333C15 3.11232 14.9122 2.90036 14.7559 2.74408C14.5996 2.5878 14.3877 2.5 14.1667 2.5H5.83333C5.61232 2.5 5.40036 2.5878 5.24408 2.74408C5.0878 2.90036 5 3.11232 5 3.33333V3.33333Z"
                        stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
                01:10:00
            </button>
            <!-- 유저 아이콘 -->
            <div class="position-relative">
                <div class="user-btn">
                    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 10 10" fill="none">
                        <path
                            d="M1.66699 5.00002C1.66699 5.88408 2.01818 6.73192 2.6433 7.35704C3.26842 7.98216 4.11627 8.33335 5.00033 8.33335C5.88438 8.33335 6.73223 7.98216 7.35735 7.35704C7.98247 6.73192 8.33366 5.88408 8.33366 5.00002C8.33366 4.11597 7.98247 3.26812 7.35735 2.643C6.73223 2.01788 5.88438 1.66669 5.00033 1.66669C4.11627 1.66669 3.26842 2.01788 2.6433 2.643C2.01818 3.26812 1.66699 4.11597 1.66699 5.00002Z"
                            stroke="#206BC4" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                    <svg xmlns="http://www.w3.org/2000/svg" width="12" height="8" viewBox="0 0 12 8" fill="none">
                        <path
                            d="M1 6.33331V4.66665C1 3.78259 1.35119 2.93475 1.97631 2.30962C2.60143 1.6845 3.44928 1.33331 4.33333 1.33331H7.66667C8.55072 1.33331 9.39857 1.6845 10.0237 2.30962C10.6488 2.93475 11 3.78259 11 4.66665V6.33331"
                            stroke="#206BC4" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </div>

                <!-- 드롭박스 -->
                <div class="dropbox">
                    <div class="dropbox-conateiner">
                        <div class="dropbox-menu">
                            <a>
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
                                    fill="none">
                                    <path
                                        d="M2.5 10C2.5 10.9849 2.69399 11.9602 3.0709 12.8701C3.44781 13.7801 4.00026 14.6069 4.6967 15.3033C5.39314 15.9997 6.21993 16.5522 7.12987 16.9291C8.03982 17.306 9.01509 17.5 10 17.5C10.9849 17.5 11.9602 17.306 12.8701 16.9291C13.7801 16.5522 14.6069 15.9997 15.3033 15.3033C15.9997 14.6069 16.5522 13.7801 16.9291 12.8701C17.306 11.9602 17.5 10.9849 17.5 10C17.5 9.01509 17.306 8.03982 16.9291 7.12987C16.5522 6.21993 15.9997 5.39314 15.3033 4.6967C14.6069 4.00026 13.7801 3.44781 12.8701 3.0709C11.9602 2.69399 10.9849 2.5 10 2.5C9.01509 2.5 8.03982 2.69399 7.12987 3.0709C6.21993 3.44781 5.39314 4.00026 4.6967 4.6967C4.00026 5.39314 3.44781 6.21993 3.0709 7.12987C2.69399 8.03982 2.5 9.01509 2.5 10Z"
                                        stroke="#6C7A91" stroke-width="1.25" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                    <path
                                        d="M7.5 8.33331C7.5 8.99635 7.76339 9.63224 8.23223 10.1011C8.70107 10.5699 9.33696 10.8333 10 10.8333C10.663 10.8333 11.2989 10.5699 11.7678 10.1011C12.2366 9.63224 12.5 8.99635 12.5 8.33331C12.5 7.67027 12.2366 7.03439 11.7678 6.56555C11.2989 6.09671 10.663 5.83331 10 5.83331C9.33696 5.83331 8.70107 6.09671 8.23223 6.56555C7.76339 7.03439 7.5 7.67027 7.5 8.33331Z"
                                        stroke="#6C7A91" stroke-width="1.25" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                    <path
                                        d="M5.13965 15.7075C5.34591 15.021 5.76796 14.4193 6.34319 13.9916C6.91842 13.564 7.61619 13.3331 8.33298 13.3333H11.6663C12.384 13.3331 13.0827 13.5645 13.6583 13.9931C14.234 14.4218 14.6559 15.0248 14.8613 15.7125"
                                        stroke="#6C7A91" stroke-width="1.25" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                </svg>
                                정보수정
                            </a>
                        </div>
                        <div class="dropbox-menu">
                            <a>
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
                                    fill="none">
                                    <path
                                        d="M11.6667 6.66665V4.99998C11.6667 4.55795 11.4911 4.13403 11.1785 3.82147C10.866 3.50891 10.442 3.33331 10 3.33331H4.16667C3.72464 3.33331 3.30072 3.50891 2.98816 3.82147C2.67559 4.13403 2.5 4.55795 2.5 4.99998V15C2.5 15.442 2.67559 15.8659 2.98816 16.1785C3.30072 16.4911 3.72464 16.6666 4.16667 16.6666H10C10.442 16.6666 10.866 16.4911 11.1785 16.1785C11.4911 15.8659 11.6667 15.442 11.6667 15V13.3333"
                                        stroke="#6C7A91" stroke-width="1.25" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                    <path d="M5.83301 10H17.4997M17.4997 10L14.9997 7.5M17.4997 10L14.9997 12.5"
                                        stroke="#6C7A91" stroke-width="1.25" stroke-linecap="round"
                                        stroke-linejoin="round" />
                                </svg>
                                로그아웃
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="table-responsive">
        <table class="table table-vcenter">
            <thead>
                <tr>
                    <th>
                    </th>
                    <th>
                        btn-gray
                    </th>
                    <th>
                        btn-blue
                    </th>
                    <th>
                        btn-red
                    </th>
                    <th>
                        btn-green
                    </th>
                    <th>
                        btn-sky
                    </th>
                    <th>
                        btn-orange
                    </th>
                    <th>
                        btn-white
                    </th>
                </tr>
            </thead>
            <tbody>
                <!-- btn-sm -->
                <tr>
                    <td>
                        btn-sm
                    </td>
                    <td>
                        <button class="btn-sm btn-gray">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-sm btn-blue">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-sm btn-red">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-sm btn-green">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-sm btn-sky">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-sm btn-orange">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-sm btn-white">
                            운영대기
                        </button>
                    </td>
                </tr>
                <!-- btn-md -->
                <tr>
                    <td>
                        btn-md
                    </td>
                    <td>
                        <button class="btn-md btn-gray">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-md btn-blue">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-md btn-red">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-md btn-green">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-md btn-sky">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-md btn-orange">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-md btn-white">
                            운영대기
                        </button>
                    </td>
                </tr>
                <!-- btn-lg -->
                <tr>
                    <td>
                        btn-lg
                    </td>
                    <td>
                        <button class="btn-lg btn-gray">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-lg btn-blue">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-lg btn-red">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-lg btn-green">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-lg btn-sky">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-lg btn-orange">
                            운영대기
                        </button>
                    </td>
                    <td>
                        <button class="btn-lg btn-white">
                            운영대기
                        </button>
                    </td>
                </tr>
            </tbody>
        </table>
    </div>

    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th>
                        편집
                    </th>
                    <th>
                        초기화
                    </th>
                    <th>
                        목록
                    </th>
                    <th>
                        등록
                    </th>
                    <th>
                        추가
                    </th>
                    <th>
                        엑셀 다운로드
                    </th>

                </tr>
            </thead>
            <tbody>
                <tr>
                    <!-- 편집 -->
                    <td>
                        <button class="btn-md btn-white">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
                                fill="none" style="margin-right: 4px">
                                <path
                                    d="M3.33337 16.6667H6.66671L15.4167 7.91669C15.8587 7.47467 16.1071 6.87515 16.1071 6.25003C16.1071 5.62491 15.8587 5.02539 15.4167 4.58336C14.9747 4.14133 14.3752 3.89301 13.75 3.89301C13.1249 3.89301 12.5254 4.14133 12.0834 4.58336L3.33337 13.3334V16.6667Z"
                                    stroke="#1D273B" stroke-width="1.25" stroke-linecap="round"
                                    stroke-linejoin="round" />
                                <path d="M11.25 5.41669L14.5833 8.75002" stroke="#1D273B" stroke-width="1.25"
                                    stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                            편집
                        </button>
                    </td>
                    <!-- 초기화 -->
                    <td>
                        <button class="btn-md btn-white">
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20"
                                fill="none" style="margin-right: 4px">
                                <path
                                    d="M16.8608 10.8675C16.7035 12.0675 16.2224 13.202 15.4693 14.1493C14.7162 15.0967 13.7194 15.8211 12.5858 16.245C11.4522 16.6689 10.2247 16.7762 9.03472 16.5555C7.84479 16.3347 6.7374 15.7942 5.8313 14.9919C4.92521 14.1896 4.2546 13.1558 3.8914 12.0014C3.5282 10.8469 3.48611 9.61538 3.76964 8.43882C4.05317 7.26226 4.65162 6.18508 5.50082 5.32279C6.35002 4.46051 7.41793 3.84565 8.59001 3.54416C11.8392 2.70916 15.2025 4.38333 16.4442 7.5"
                                    stroke="#1D273B" stroke-width="1.25" stroke-linecap="round"
                                    stroke-linejoin="round" />
                                <path d="M16.9167 3.33331V7.49998H12.75" stroke="#1D273B" stroke-width="1.25"
                                    stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                            초기화
                        </button>
                    </td>
                    <!-- 목록 -->
                    <td>
                        <button class="btn-md btn-white">
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20"
                                fill="none" style="margin-right: 4px">
                                <path d="M7.75 5H16.9167" stroke="#1D273B" stroke-width="1.25" stroke-linecap="round"
                                    stroke-linejoin="round" />
                                <path d="M7.75 10H16.9167" stroke="#1D273B" stroke-width="1.25" stroke-linecap="round"
                                    stroke-linejoin="round" />
                                <path d="M7.75 15H16.9167" stroke="#1D273B" stroke-width="1.25" stroke-linecap="round"
                                    stroke-linejoin="round" />
                                <path d="M4.41663 5V5.00833" stroke="#1D273B" stroke-width="1.25" stroke-linecap="round"
                                    stroke-linejoin="round" />
                                <path d="M4.41663 10V10.0083" stroke="#1D273B" stroke-width="1.25"
                                    stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M4.41663 15V15.0083" stroke="#1D273B" stroke-width="1.25"
                                    stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                            목록
                        </button>
                    </td>
                    <!-- 등록 -->
                    <td>
                        <button class="btn-md btn-white">
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20"
                                fill="none" style="margin-right: 4px">
                                <path d="M10.25 4.16669V15.8334" stroke="#1D273B" stroke-width="1.25"
                                    stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M4.41663 10H16.0833" stroke="#1D273B" stroke-width="1.25"
                                    stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                            등록
                        </button>
                    </td>
                    <!-- 추가 -->
                    <td>
                        <button class="btn-md btn-white">
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20"
                                fill="none" style="margin-right: 4px">
                                <path d="M10.25 4.16669V15.8334" stroke="#1D273B" stroke-width="1.25"
                                    stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M4.41663 10H16.0833" stroke="#1D273B" stroke-width="1.25"
                                    stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                            추가
                        </button>
                    </td>
                    <!-- 엑셀 다운로드 -->
                    <td>
                        <button class="btn-md btn-white">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"
                                fill="none" style="margin-right: 4px">
                                <path
                                    d="M11.6666 2.5V5.83333C11.6666 6.05435 11.7544 6.26631 11.9107 6.42259C12.067 6.57887 12.2789 6.66667 12.5 6.66667H15.8333"
                                    stroke="#1D273B" stroke-width="1.25" stroke-linecap="round"
                                    stroke-linejoin="round" />
                                <path
                                    d="M14.1666 17.5H5.83329C5.39127 17.5 4.96734 17.3244 4.65478 17.0118C4.34222 16.6993 4.16663 16.2754 4.16663 15.8333V4.16667C4.16663 3.72464 4.34222 3.30072 4.65478 2.98816C4.96734 2.67559 5.39127 2.5 5.83329 2.5H11.6666L15.8333 6.66667V15.8333C15.8333 16.2754 15.6577 16.6993 15.3451 17.0118C15.0326 17.3244 14.6087 17.5 14.1666 17.5Z"
                                    stroke="#1D273B" stroke-width="1.25" stroke-linecap="round"
                                    stroke-linejoin="round" />
                                <path d="M10 14.1666V9.16663" stroke="#1D273B" stroke-width="1.25"
                                    stroke-linecap="round" stroke-linejoin="round" />
                                <path d="M7.91663 12.0834L9.99996 14.1667L12.0833 12.0834" stroke="#1D273B"
                                    stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                            <p>

                                엑셀 다운로드
                            </p>
                        </button>
                    </td>
                </tr>
            </tbody>
        </table>
    </div>

</body>

</html>