                                    ...
                                    <td colspan="3" class="no-border-bottom">
                                        <div class="form-inline" >
                                            <?php
                                                ...
                                                $extras   = ['id' => 's_view_gbn', 'class' => 'form-select', 'style' => 'max-width: 150px;margin-right:0.235em;', 'onChange'=>'changeLocation( this.value, \'#s_locate\' )'];
                                                $selected = '';
                                                echo getSelectBox('s_view_gbn', $options, $selected, $extras);
                                            ?>
                                            <?php
                                                $options  = [''=>'노출분류 선택'];
                                                $extras   = ['id' => 's_locate', 'class' => 'form-select', 'style' => 'max-width: 150px;margin-right:0.235em;'];
                                                $selected = '';
                                                echo getSelectBox('s_locate', $options, $selected, $extras);
                                            ?>
                                        </div>
...
<!-- Modal E-->
<script>
function openLayer( bannerIdx, id ){
    let data = '';
    let url  = '/apis/design/bannerRegister';

    if( bannerIdx != '' ){

        data = 'banner_idx='+bannerIdx;
        url  = '/apis/design/bannerDetail';
    }
    $.ajax({
        url: url,
        type: 'post',
...
let options = <?php echo json_encode( _elm( $aConfig, 'viewLoc' ) )?>;

function changeLocation(selectedValue, targetId ) {
    $(targetId).empty();
    if (selectedValue in options) {
        let selectedOptions = options[selectedValue];
        for (let key in selectedOptions) {
            if (selectedOptions.hasOwnProperty(key)) {
                $(targetId).append(new Option(selectedOptions[key], key));
            }
        }
    } else {
        // 기본 옵션 추가 (예: 전체)
        $(targetId).append(new Option('노출분류 선택', ''));
    }
}
...
<?php
$owensView->setFooterJs('/assets/js/design/banner/lists.js');
$owensView->setFooterJs('/assets/js/design/banner/register.js');
$owensView->setFooterJs('/assets/js/design/banner/detail.js');
