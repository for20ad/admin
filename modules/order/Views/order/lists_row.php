...
<?php
    ...
    if (empty($aLists) === false)
    {
        ...
        foreach ($aLists as $vData)
        {
            ...
            $no = $total_rows - $row + 1;
?>
<tr data-idx="<?php echo _elm( $vData , 'B_IDX' )?>" style="height:36px;">

    <td class="body2-c small-font"><?php echo $no?></td>
    <td class="body2-c small-font"><a href='javascript:;' onclick="openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')"><?php echo _elm( $vData , 'O_ORDER_AT' ) ?></a></td>
    ...
    <?php echo _elm( $vData, 'IS_FIRST_ORDER') == true? '<div style="display: flex;padding: 4px;justify-content: center;align-items: center;gap: 8px;"><span style="padding:2px 2px;background: var(--sns---tblr-facebook, #1877F2);color:#fff">첫주문</span></div>' : '' ?>
        <a href='javascript:;' onclick="openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')"><?php echo _elm( $vData, 'O_ORDID' )?></a>
    </td>
    <td class="body2-c small-font"><a href='javascript:;' onclick="openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')"><?php echo _elm( $vData, 'MB_NM' )?></a></td>
    <td class="body2-c small-font" style="text-align:left"><a href='<?php echo base_url().'order/orderDetail/'._elm( $vData , 'O_IDX' )?>'><?php echo _elm( $vData , 'O_TITLE' )?></a></td>
    <td class="body2-c small-font"><?php echo number_format( _elm( $vData , 'O_TOTAL_PRICE' ) ) ?></td>
    <td class="body2-c small-font"><?php echo number_format( _elm( $vData, 'O_SHIP_PRICE', 0 ) )?></td>
    <td class="body2-c small-font"><?php echo number_format( _elm( $vData, 'O_PG_PRICE' ) )?></td>
    <td class="body2-c small-font"><?php echo _elm( $vData, 'O_STATUS' ) == 'Created'? '미주문' : _elm( $aPayMethods, _elm( $vData, 'O_PAY_METHOD' ) ) ?></td>
    <td class="body2-c small-font"><?php echo _elm( $aOrderStatus, _elm( $vData, 'O_STATUS' ) ) ?></td>
    <td class="body2-c small-font" style="background: var(--Greyscale---tblr-gray-100, #F1F5F9);">
        <a href='javascript:;' onclick="javascript:openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')">
            <?php echo _elm( $vData, 'readyCnt' ) ?>
        </a>
    </td>
    <td class="body2-c small-font" style="background: var(--Greyscale---tblr-gray-100, #F1F5F9);">
        <a href='javascript:;' onclick="javascript:openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')">
            <?php echo _elm( $vData, 'ingCnt' ) ?>
        </a>
    </td>
    <td class="body2-c small-font" style="background: var(--Greyscale---tblr-gray-100, #F1F5F9);">
        <a href='javascript:;' onclick="javascript:openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')">
            <?php echo _elm( $vData, 'complateCnt' ) ?>
        </a>
    </td>
    <td class="body2-c small-font" style="background: var(--Greyscale---tblr-gray-100, #F1F5F9);">
        <a href='javascript:;' onclick="javascript:openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')">
            <?php echo _elm( $vData, 'cancelCnt' ) ?>
        </a>
    </td>
    <td class="body2-c small-font" style="background: var(--Greyscale---tblr-gray-100, #F1F5F9);">
        <a href='javascript:;' onclick="javascript:openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')">
            <?php echo _elm( $vData, 'exchangeCnt' ) ?>
        </a>
    </td>
    <td class="body2-c small-font" style="background: var(--Greyscale---tblr-gray-100, #F1F5F9);">
        <a href='javascript:;' onclick="javascript:openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')">
            <?php echo _elm( $vData, 'returnCnt' ) ?>
        </a>
    </td>
    <td class="body2-c small-font" style="background: var(--Greyscale---tblr-gray-100, #F1F5F9);">
        <a href='javascript:;' onclick="javascript:openLayer('<?php echo _elm( $vData , 'O_IDX' ) ?>', 'dataModal')">
            <?php echo _elm( $vData, 'refundCnt' ) ?>
        </a>
    </td>
