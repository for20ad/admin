<?php
    ...
    $orderBtns        = [
        'PayComplete'                      => '결제완료',
        'RefundRequest'                    => '환불요청대기',
        'ProductWaiting'                   => '상품준비중',
        'ShippingWaiting'                  => '배송 준비 중',
        'ShippingProgress'                 => '배송중',
        'ShippingComplete'                 => '배송완료',
        'ShippingCancelRequest'            => '배송결제취소예정',
        'ShippingCancelComplete'           => '배송결제취소완료',
        'ShippingRefundRequest'            => '배송환불요청대기',
        'ShippingRefundWaiting'            => '배송환불예정',
        'ShippingRefundFailed'             => '배송환불재요청대기',
        'ShippingRefundComplete'           => '배송환불완료',
        'ShippingCancelRequestBuyer'       => '구매자 배송취소요청',
        'ShippingCancelRequestSeller'      => '판매자 배송취소요청',
        'ShippingCancelRejected'           => '취소불가(기발송)',
        'ShippingCancelRequestRetract'     => '취소요청철회',
        'BuyDecision'                      => '구매확정',
    ];
    ...
    $aOrderStatus = [];
    foreach (_elm( $orderStatus, 'complete' ) as $statusKey) {
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <input type="hidden" name="s_ordStatus" id="s_ordStatus" value="PayComplete,RefundRequest">
...
<div class="container-fluid" style="width: 100%; overflow-x: hidden;">
    <div class="col-12 col">
        ...
        <div class="card accordion-card" style="padding: 17px 24px; border-top:0; border-bottom: 0">
            ...
            <div class='tab-wrap'>
                <ul class="nav nav-tabs" id="orderStatusTabs" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <a class="nav-link <?php echo $i== 1? 'active' : '' ?>" id="<?php echo $statusKey?>-tab"
                            data-bs-toggle="tab" href="#<?php echo $statusKey?>" role="tab"
                            aria-controls="<?php echo $statusKey?>"
                            aria-selected="true"><?php echo _elm( $orderTitle, $statusKey)?><span
                                id="<?php echo $statusKey?>Cnt"></span>
                        </a>
                    </li>
                    <?php
                            endforeach;
                ...
                <?php
                        ...
                        if( empty( $orderStatus ) == false ):
                            foreach ($orderStatus as $statusKey => $statusValues):
                                $i++;
                    ?>
                <div class="tab-pane fade show <?php echo $i == 1 ? 'active' : '' ?>" id="<?php echo $statusKey?>"
                    role="tabpanel" aria-labelledby="<?php echo $statusKey?>-tab"
                    style="background: white; padding-inline: 24px; padding-bottom:16px; overflow-y: visible">
                    <div class="table-responsive order-table" style="overflow-x: auto; min-width: 1200px;">
                        <table class="table table-vcenter" id="<?php echo $statusKey?>OrdersTable"
                            style="min-width: 1200px; width: 100%; table-layout: fixed; word-wrap: break-word;">
                            <colgroup>
...
function openLayer(ordIdx, id) {
    ...
    data = 'ordIdx=' + ordIdx;
    url = '/apis/order/orderStatusOverDetail';
    $.ajax({
        url: url,
        type: 'post',
        data: data,
...
<?php
$owensView->setFooterJs('/assets/js/order/payStatusOver/lists.js');
$owensView->setFooterJs('/assets/js/order/payStatusOver/register.js');
$owensView->setFooterJs('/assets/js/order/payStatusOver/detail.js');
