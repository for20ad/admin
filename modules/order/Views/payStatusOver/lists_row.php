...
<tr data-idx="<?php echo _elm($vData, 'B_IDX') ?>">
    ...
    <td class="body2-c small-font">
        <span
            class="status-box <?php echo _elm( $statusTextClass, _elm($vData, 'O_STATUS') )?>"><?php echo _elm($aOrderStatus, _elm($vData, 'O_STATUS')); ?></span>
    </td>
    ...
    <td class="body2-c small-font">
        ...
        <div class="form-inline" style="padding:0;margin:0">
            <input type="text" name="i_ship_number" class="form-control"
                value="<?php echo _elm( $vData, 'O_SHIP_NUMBER' ) ?>"
                data-orgShipNum="<?php echo _elm( $vData, 'O_SHIP_NUMBER' )?>"
                data-idx="<?php echo _elm($vData, 'O_IDX')?>" onblur="if (!window.preventOnBlur) {
    ...
    <td class="body2-c small-font" style="text-align:left">
        <?php echo _elm($vData, 'P_NAME'). ' '._elm($vData, 'P_OPTION_NAME')  ?></td>

    <?php if ($previousOrderId !== $currentOrderId): ?>
    <td class="body2-c small-font" rowspan="<?php echo $orderRowspanCount[$currentOrderId]; ?>">
        <?php echo number_format(_elm($vData, 'O_TOTAL_PRICE')) ?></td>
    <td class="body2-c small-font" rowspan="<?php echo $orderRowspanCount[$currentOrderId]; ?>">
        <?php echo number_format(_elm($vData, 'O_CPN_MINUS_PRICE', 0) + _elm($vData, 'O_CPN_PLUS_MINUS_PRICE', 0)) ?>
    </td>
    <td class="body2-c small-font" rowspan="<?php echo $orderRowspanCount[$currentOrderId]; ?>">
        <?php echo number_format(_elm($vData, 'O_SHIP_PRICE', 0) + _elm($vData, 'O_ADD_SHIP_PRICE', 0)) ?></td>
    <td class="body2-c small-font" rowspan="<?php echo $orderRowspanCount[$currentOrderId]; ?>">
        <?php echo number_format(_elm($vData, 'O_PG_PRICE')) ?></td>
    <td class="body2-c small-font" rowspan="<?php echo $orderRowspanCount[$currentOrderId]; ?>" id="<?php echo $no; ?>">
        <?php
                $extras = [
                ...
                if (!empty(_elm($vData, 'aMemo'))) {
                    $extras += [
                        'onmouseover' => '$(this).closest(\'tr\').find(\'#aLastMemo-' . $no . '\').show()',
                        'onmouseout' => '$(this).closest(\'tr\').find(\'#aLastMemo-' . $no . '\').hide()',
                    ];
        ...
        <?php
                ...
                ?>
        <div>
            <div id="aLastMemo-<?php echo $no; ?>" class="memo-layer"
                style="width:650px;display: none; position: absolute; right:8.2rem; background: #fff; border: 1px solid #ccc; padding: 10px; margin-top:6px">
                <!-- Header -->
                <div class="memo-header"
                    style="display: flex; font-weight: bold; border-bottom: 1px solid #ccc; padding-bottom: 5px; margin-bottom: 5px;">
                ...
                <div class="memo-content" style="display: flex; padding-top: 5px;">
                    <div style="flex: 2; text-align: center; box-sizing: border-box; padding: 5px;">
                        <?php echo _elm($aMemo, 'M_CREATE_AT'); ?></div>
                    <div style="flex: 1.2; text-align: center; box-sizing: border-box; padding: 5px;">
                        <?php echo _elm($aMemo, 'M_WRITER_NAME'); ?></div>
                    <div style="flex: 1; text-align: center; box-sizing: border-box; padding: 5px;">
                        <?php echo _elm( $aOrderStatus, _elm($aMemo, 'M_STATUS') ); ?></div>
                    <div style="flex: 1.3; text-align: center; box-sizing: border-box; padding: 5px;">
                        <?php echo str_replace( ',', '<br>',_elm($aMemo, 'M_ORD_INFO_TXT') ); ?></div>
                    <div style="flex: 3; text-align: center; box-sizing: border-box; padding: 5px;">
                        <?php echo _elm($aMemo, 'M_CONTENT'); ?></div>
                </div>
            </div>
        </div>
        <?php
...
<script>
    $(function(){
            ...
            const memos = document.querySelectorAll('.memo-layer');
            memos.forEach(memo => {
                const targetId = memo.id.split('-')[1];
                const targetElement = document.getElementById(targetId);

                if (targetElement) {

                    const rect = targetElement.getBoundingClientRect();
                    memo.style.top = `${window.innerHeight - rect.top}px`;
