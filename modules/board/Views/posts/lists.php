<?php
    ...
    $aBoardInfo       = _elm( $pageDatas, 'boardInfo', [] );
    $aGetParam        = _elm( $pageDatas, 'getParam', [] );
?>
...
<div class="container-fluid">
    ...
    <div class="card-title">
        <h3 class="h3-c"><?php echo _elm( $aBoardInfo, 'B_TITLE' )?> </h3>
    </div>
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <input type="hidden" name="bo_idx" value="<?php echo _elm( $aBoardInfo, 'B_IDX' )?>">
                    <input type="hidden" name="bo_id" value="<?php echo _elm( $aBoardInfo, 'B_ID' )?>">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td colspan="3" class="no-border-bottom">
                                        <div class="form-inline">
                                            <?php
                                                $options = ['title'=>'제목','content'=>'내용','writer_name'=>'회원명','writer_idx'=>'회원반호'];
                                                $extras   = ['id' => 's_condition', 'class' => 'form-select', 'style' => 'max-width: 150px;margin-right:0.235em;'];
                                                $selected = _elm( $aGetParam, 's_condition' );
                                                echo getSelectBox('s_condition', $options, $selected, $extras);
                                            ?>
                                            <input type="text" class="form-control" style="width:15.2rem" name="s_keyword" id="s_keyword" value="<?php echo _elm( $aGetParam, 's_keyword' )?>">
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    <th class="no-border-bottom">등록일</th>
                                    <td colspan="3" class="no-border-bottom">
                                        <div class="form-inline">
                                            <div style="">
                                            <?php
                                ...
                                <tr>
                                    ...
                                    <?php
                                        ...
                                        $options = [''=>'전체', '1'=>'노출', '2'=>'비노출','3'=>'삭제','9'=>'블라인드'];
                                ...
                                <?php
                                if( _elm( $aBoardInfo, 'B_ID' ) == 'QNA' ){
                                ?>
                                <tr>
                                    <th class="no-border-bottom">답변 상태</th>
                                    <td colspan="3" class="no-border-bottom">
                                    <?php
                                        $options = [''=>'전체', 'READY'=>'문의 등록', 'RECEIVED'=>'문의 접수','PREPARING'=>'답변 준비중','COMPLETED'=>'답변 완료'];
                                        $extras   = ['id' => 's_answer_status', 'class' => 'form-select', 'style' => 'max-width: 150px;margin-right:0.235em;'];
                                        $selected = '';
                                        echo getSelectBox('s_answer_status', $options, $selected, $extras);
                                    ?>
                                    </td>
                                </tr>
                                <?php
                                ...
                                ?>
                                <tr>
                                    <th class="no-border-bottom" style="padding-bottom:0;">공지</th>
                                ...
                                <tr>
                                    <th class="no-border-bottom" style="margin-top:0">구분</th>
                                    <td colspan="3" class="no-border-bottom" style="margin-top:0">
                                        <div class="input-group" style="padding-top:0.45rem;margin-top:0">
                                        <?php
                                            $checked = false;
...
<script>
    ...
    var contents_editor = '';
    var answer_editor = '';
    function openLayer( pIdx, id ){
        let data = 'b_id=<?php echo _elm( $aBoardInfo, 'B_ID' )?>';
        let url  = '/apis/board/postsRegister';
        var editor_ids = ['contents_editor', 'answer_editor'];

        if( pIdx != '' ){
            data = '&n_idx='+pIdx;
            url  = '/apis/board/postsDetail';
        }
        $.ajax({
            url: url,
            type: 'post',
    ...
    // 더블클릭으로 .newStatus를 표시하는 함수
    function showNewStatus(element) {
        // 현재 요소(orgStatus) 숨기고, newStatus 보이기
        $(element).hide();
        $(element).parent().find('.newStatus').show();

        // ESC 키 이벤트 등록 (현재 문서에서만)
        $(document).on('keyup.hideNewStatus', function (event) {
            if (event.key === 'Escape') {
                // newStatus 숨기기
                $(element).parent().find('.newStatus').hide();
                // orgStatus 다시 표시
                $(element).show();
                // 이벤트 핸들러 해제
                $(document).off('keyup.hideNewStatus');
            }
        });
    }
</script>
...
<?php
    $owensView->setFooterJs('/assets/js/board/posts/lists.js');
    $owensView->setFooterJs('/assets/js/board/posts/register.js');
    $owensView->setFooterJs('/assets/js/board/posts/detail.js');
