...
<tr data-idx="<?php echo _elm( $vData , 'F_IDX' )?>">
    ...
    <td class="body2-c nowrap"><?php echo _elm( $aCate, _elm( $vData, 'F_CATE' ) );?> </td>
    <td class="body2-c nowrap"><a href='javascript:;' onclick="javascript:openLayer('<?php echo _elm( $vData , 'F_IDX' ) ?>', 'dataModal')"><?php echo _elm( $vData , 'F_TITLE' ) ?></a></td>
    <td class="body2-c nowrap">
        <?php echo
            _elm( $vData, 'F_STATUS' ) == 'Y' ? '<span class="status-box status-normal">노출</span>' :
            ( _elm( $vData, 'F_STATUS' ) == 'N' ? '<span class="status-box status-pending">미노출</span>' :
            ( _elm( $vData, 'F_STATUS' ) == 'D' ? '<span class="status-box status-deleted">삭제</span>' : '<span class="status-box status-disabled">대기</span>' )   )
        ?>
    </td>
    <td class="body2-c nowrap">
        <?php
            for ($i = 1; $i <= 2; $i++) {
            $filePath = _elm($vData, "F_FILES_{$i}_PATH");
            if (!empty($filePath)) {
                echo '
    ...
    <td class="body2-c nowrap"><?php echo empty( _elm( $vData , 'F_CREATE_AT' ) ) == false ? date( 'Y-m-d' , strtotime( _elm( $vData , 'F_CREATE_AT' ) ) ) : '-' ?></td>
    <td class="body2-c nowrap"><?php echo empty( _elm( $vData , 'F_UPDATE_AT' ) ) == false ? date( 'Y-m-d' , strtotime( _elm( $vData , 'F_UPDATE_AT' ) ) ) : '-' ?></td>
    <td class="body2-c nowrap"><?php echo _elm( $vData, 'F_HIT' ) ?? 0?></td>
    <td class="body2-c nowrap">
    <?php
        echo getIconAnchor([
            'txt' => '',
            ...
            'stroke' => '#616876',
            'extra' => [
                'onclick' => 'deleteFaqConfirm("'. _elm( $vData , 'F_IDX' ) .'");',
            ]
        ]);
    ?>
    </td>
